...
setInterval(() => client.user.setActivity({ name: `${config.status} - bu parça ${Number(client?.shard?.ids)+1 ? Number(client?.shard?.ids)+1 : "1"}`, type: ActivityType.Listening }), 10000);
...
http.createServer(function (req, res){
  res.write("Botunuz GÃ¼venli Bir Åekilde 7/24 OlmuÅŸtur");
  res.end();
}).listen(8080);
